...
| 7 | `Step 7: Read Security Notes` | `Before entering keys, review Security Notes in a new tab: <a href="https://agentic-ethereum.com/about#security-notes" target="_blank" rel="noopener noreferrer">Security Notes</a>. This tutorial states that the raw value of Confidential Keys is not stored on the server.` | `[data-tour="agent-security-notes-link"]` | Security Notes link click event detected (`hasOpenedSecurityNotes === true`) | No |
