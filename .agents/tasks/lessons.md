...
- When users request visual cleanup of SNS header/nav, adjust alignment via shared layout containers (not one-off per-link tweaks) and enforce any explicitly requested menu order (e.g., `Docs` rightmost) exactly.
- For SNS Docs UX requests with strict layout constraints, implement a route-specific minimal chrome path (no global header/footer badges) and match the exact section/subsection names the user specifies.
...
- For block-to-block security diagrams, annotate both source and destination explicitly on each arrow label line (not just arrow direction), especially when one destination block appears elsewhere in the layout.
...
- In mixed-direction block diagrams, keep connector shafts continuous from source block to destination block; place `^` on a shaft row, not as a detached top marker.
...
- When the user explicitly requests committing all workspace changes, include unrelated pre-existing edits in the same commit instead of splitting or excluding files.
...
- 사용자가 "회전 + 중심에서 바깥 확산"을 명시하면 컨테이너 회전만으로 처리하지 말고, 입자별 외향 이동 벡터를 넣어 실제 나선형 방사 애니메이션을 구현해야 한다.
- 사용자가 특정 실험 페이지를 제거하고 배경만 감상 가능한 화면을 원하면, 해당 페이지/네비를 완전히 제거하고 크롬 없는 전용 빈 라우트를 제공해야 한다.
- 사용자가 페이지가 흰 화면이라고 보고하고 경로 표기에 오타가 보이면(예: `backround`), 오타 별칭 라우트도 함께 지원해 진입 실패를 즉시 차단해야 한다.
- 사용자가 별칭 경로 제거를 명시하면, 임시 호환 라우트(오타 alias)를 즉시 삭제하고 공식 경로만 유지해야 한다.
- 사용자가 3D 깊이감을 요구할 때는 컨테이너 레벨 확대만이 아니라, 입자별 거리 기반 크기 성장 계수(중심->외곽)를 애니메이션에 직접 반영해야 한다.
- 사용자가 '모든 점이 나선 방향으로'를 요구하면 직선 drift 기반 이동을 버리고, 각도+반지름(극좌표) 보간으로 전체 입자(일반/밝은/별자리) 경로를 통일해야 한다.
- 사용자 피드백이 광원 단층(밴딩) 이슈일 때는 큰 그라디언트 면에 디더링/미세 노이즈/중간 톤 레이어를 추가해 전이 경계를 깨야 한다.
- 밴딩 수정 후에도 화면 깨짐(화이트 seam)이 보이면 mix-blend/대형 blur pseudo-overlay를 제거하고, 기본 그라디언트+저강도 노이즈 조합으로 컴포지팅 부담을 낮춰야 한다.
- 사용자가 특정 로고(예: 이더리움 마크) 형상이 틀렸다고 지적하면, 근사 좌표를 임의 조정하지 말고 표준 꼭짓점/엣지 모델을 정의해 별자리 점 생성과 오버레이 선 렌더를 동일 좌표계로 통일해야 한다.
- 사용자가 오버랩 라인이 실제 로고와 다르다고 재지적하면, 단일 중심점 기반 도형을 고집하지 말고 상/하 중심점을 분리한 실제 와이어프레임 위상으로 재정의해야 한다.
- 사용자가 광원 효과 전체 제거를 요구하면, 중심 광채 레이어뿐 아니라 별 글로우(box-shadow), 플레어 pseudo-element, glyph drop-shadow/blur, brightness twinkle까지 함께 제거해야 한다.
